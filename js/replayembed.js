...
var Replays = {
	...
	init: function () {
        ...
        if (window.FAKEMON_SPRITES) {
            const fakemonMap = window.FAKEMON_SPRITES;
            const origGetSprite = this.battle.getSpriteUrl;
            this.battle.getSpriteUrl = function(pokemon, isBack) {
                const id = (pokemon.species || '').toLowerCase();
                if (fakemonMap[id]) {
                    const s = fakemonMap[id];
                    return isBack ? s.back : s.front;
                }
                return origGetSprite.call(this, pokemon, isBack);
            };
        }
